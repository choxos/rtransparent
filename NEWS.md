...
Release-integrity and metadata polish (no change to the detectors or API).

* Added machine-readable FAIR metadata to the repository: `CITATION.cff`,
  `codemeta.json`, and `ro-crate-metadata.json`, plus a `LICENSE.note`
  provenance file and a test-coverage workflow. All are excluded from the CRAN
  source tarball.
...
* Removed the pre-publication CRAN status badge from the README (it will be
  added once the package is on CRAN).
