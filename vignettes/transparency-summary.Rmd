...
Replication and AI-use disclosure have no bundled accuracy estimates here, so
their corrected values are `NA`. Novelty's estimate comes from a hand-labeled
gold set (`inst/benchmark/results_novelty_replication.md`); the data/code values
are reproducible benchmark estimates for the native detector, not untouched
external-validation estimates.
