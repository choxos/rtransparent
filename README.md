...
detector's specificity remains high. The native **data sensitivity (77%) is
still below `oddpub`'s ~84%** on this set, on a tail of supplement-only data,
reused public-source statements and "available upon request" data that the
detector deliberately excludes as not openly shared; treat the native data
